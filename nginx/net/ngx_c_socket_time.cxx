...
LPSTRUC_MSG_HEADER CSocekt::GetOverTimeTimer(time_t cur_time)
{
    ...
    if (earliesttime <= cur_time)
    {
        //这回确实是有到时间的了【超时的节点】
        ptmp =
            RemoveFirstTimer();  //把这个超时的节点从 m_timerQueuemap 删掉，并把这个节点的第二项返回来；
            ...
            1)  //能调用到本函数第一个条件肯定成立，所以第一个条件加不加无所谓，主要是第二个条件
        {
            ...
            LPSTRUC_MSG_HEADER tmpMsgHeader =
                (LPSTRUC_MSG_HEADER) p_memory->AllocMemory(sizeof(STRUC_MSG_HEADER), false);
    ...
    while (g_stopEvent == 0)  //不退出
    {
        ...
        if (pSocketObj->m_cur_size_ > 0)  //队列不为空，有内容
        {
            ...
            if (absolute_time < cur_time)
            {
                ...
                if (err != 0)
                    ngx_log_stderr(err,
                                   "CSocekt::ServerTimerQueueMonitorThread()中pthread_mutex_lock()"
                                   "失败，返回的错误码为%d!",
                                   err);  //有问题，要及时报告
                ...
                if (err != 0)
                    ngx_log_stderr(err,
                                   "CSocekt::ServerTimerQueueMonitorThread()pthread_mutex_unlock()"
                                   "失败，返回的错误码为%d!",
                                   err);  //有问题，要及时报告
                ...
                while (!m_lsIdleList.empty())
                {
                    ...
                    pSocketObj->procPingTimeOutChecking(tmpmsg,
                                                        cur_time);  //这里需要检查心跳超时问题
